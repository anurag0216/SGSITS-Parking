...
$req_id = $_GET['request_id'];
    $req = "SELECT `requests`.`id`, `parking_id`, `customer`, `slots`, `hours`, `cost`, `time`, `status`,`location`, `street`, `name` FROM `requests`,`parkings` WHERE `requests`.`parking_id`=`parkings`.`id` AND `requests`.`id`='$req_id '  ";
...
// $charge = (float)($hours * $cost/2);
	
    if($stat=='requested'){
		    $otp = (string)(rand(1000,9999));
			// echo($otp);
            $update_request = "UPDATE `requests` SET `status`='Completed', `otp`='$otp'  WHERE `id`='$id'";
			...
			SendOtp($request['customer'], $otp);
    }
    // echo($request['customer']);
    echo '<div style="background-color: green; color: white; padding: 10px;">' . $request['customer'] . '</div>';
